...
class CamObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, edge_root, trainsize):
        ...
        self.edges = [edge_root + f for f in os.listdir(edge_root) if f.endswith('.npy')]
    ...
    def __getitem__(self, index):
        ...
        edge = np.load(self.edges[index], allow_pickle=True).item()
        ...
        gt = Image.fromarray(gt_arr.astype(np.uint8))
        gt = self.ge_transform(gt)

        edge = edge["hough_space"].astype(np.float32)
